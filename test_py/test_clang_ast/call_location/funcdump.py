...
if __name__ == "__main__":
    path = r'D:\PyCharmTest\PyCharmPackets\Models\StaticCodeAnalyzer\FastCodeReview\test\init_data\test_data\test_c_\test_headers.cpp'
    # obj = FunctionPreprocessor(path, 'add')
    combiner = DefinitionCallExpressCombiner(path)
    temp_filepath = combiner.Combiner()
    call_analyzer = FunctionDump(temp_filepath)
    call_analyzer.analyseLauncher()
    call_analyzer.show_function_details()
